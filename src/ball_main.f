...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE MAINDRIVER_BALL (tot_conc,time_out,D00,Ea0,nt0,
     *                     therm_hist0,ages,conc_hist,nnode0,rad0,
     *                     alpha_ejec0,diffuse,min_diff)
!
! main program
!
      USE direct_procedures_ball
      IMPLICIT NONE
!
...
C local variables
      ...
      REAL(KIND=r8) :: perim,D00,Ea0,time_out(0:nt0)
      REAL(KIND=r8) :: elem_conc,conc_hist(nnode0)
      REAL(KIND=r8) :: tot_conc(nt0),ages(nt0)
      REAL(KIND=r8) :: const_rate,var_rate
      REAL(KIND=r8) :: vol1,vol2,rad0,summ,min_diff

      INTEGER :: err,i,j,k,nt0,nnode0,ft_count,alpha_ejec0
      CHARACTER i_char*4,therm_hist0*100,concFlag*3
      INTEGER :: diffuse,diffuse2,counter,store(100),tempStore,jt

      REAL(KIND=r8),DIMENSION(3,3) :: E,F

      write(6,*) 'Processing 3-D ball'
!       constant=4.0e-14
      rad=rad0                          ! Radius of sphere
...
!      CALL QEB_BALL(A,ut(1,:),ut(0,:))
      ...
      DO it=1,nt
         ...
         if (diffuse2 .eq. 1) then
           CALL QEB_BALL(A,ut(it,:),ut(it-1,:))
           CALL BDE_BALL(A,ut(it,:))
           CALL CRS_BALL(A,W)
           CALL QMR_BALL(W,ut(it,:))
           jt=it
         else
           ut(it,1:nnode)=ut(jt,1:nnode)+ft(1:nnode)*(it-jt)*dt
         endif

         WRITE(*,*) 'time step:', it,qt(it)
      END DO


      ! Scale sphere back to original dimensions
      xnode=xnode*rad
      ynode=ynode*rad
      znode=znode*rad
      volum=volum*rad**3.
...
!       tot_volum=(4./3.)*pi*rad**3.
      tot_volum=0.
      do i=1,nelem
        tot_volum=tot_volum+volum(i)
      enddo

      ft_count=0                                ! Amount of points less than 20 microns from edge of sphere
      ...
      vol1 = (4./3.)*pi*(rad-20.e-6_r8)**3.     ! Volume of inner sphere that contains points greater than 20 microns from edge
      vol2 = tot_volum-vol1                     ! Volume of outer shell (space between the inner sphere and total sphere)
      ...
      ! Total production rate is the ratio of the inner sphere at the constant rate
     ...
     *                  (vol2/tot_volum)*var_rate
      ...
      ! in the sphere for each timestep
      DO i=1,nt
        ...
        DO j=1,nelem
          ...
          END DO
          elem_conc = 0.25*elem_conc*volum(j)
          tot_conc(i) = tot_conc(i) + elem_conc
        END DO

        ! Calculate the sphere age by taking the total Helium volume
        ! and dividing by the total sphere volume to get a concentration
        ! and then dividing by the total production rate in the sphere
        ...
        OPEN(unit=80,file='ball/He_tec'//i_char//'.dat',
     *       status='unknown')

        write (80,'(A)',ADVANCE="no") 'TITLE = "bambHe Output -'
        write (80,*) ' Ball - Time: ',time(i),'s"'
        ...
        write (80,*) 'ZONE T = "Ball - Time: ',time(i),'s"'
...
!       open (81,file="conc_history.dat",status="unknown")
! 
!       counter=0
!       do i=1,nnode
!         if (ynode(i).lt.1d-5 .and. ynode(i).ge.0. .and. 
!      *      znode(i).lt.1d-5 .and. znode(i).ge.0. .and.
!      *      xnode(i).ge.0.) then
!           counter=counter+1
!           store(counter)=i
!         endif
...
!       do i=1,counter
!         do j=i+1,counter
!           if (xnode(store(i)).gt.xnode(store(j))) then
!             tempStore=store(i)
!             store(i)=store(j)
!             store(j)=tempStore
!           endif
...
! 
!       do i=1,counter
!         write (81,*) xnode(store(i)),ut(nt,store(i))
!       enddo
...
C clean up variables
      ...
      RETURN
      END SUBROUTINE MAINDRIVER_BALL
...
C read data
      ...
      OPEN(unit=81,file='ball/nnes_ball.dat')
...
C read data from Matlab
      ...
      OPEN(unit=82,file='ball/xyzn_ball.dat')
      DO i=1,nnode
	 READ(82,*) xnode(i),ynode(i),znode(i)
      END DO
      CLOSE(unit=82)

      OPEN(unit=83,file='ball/node_ball.dat')
      DO i=1,nelem
         READ(83,*) gnode(1,i),gnode(2,i),gnode(3,i),gnode(4,i)
      END DO
      CLOSE(unit=83)

      OPEN(unit=84,file='ball/sfnd_ball.dat')
      DO i=1,nsfem
	 READ(84,*) gsfnd(1,i),gsfnd(2,i),gsfnd(3,i)
      END DO
      CLOSE(unit=84)

      OPEN(unit=85,file='ball/volu_ball.dat')
      DO i=1,nelem
	 READ(85,*) volum(i)
!          tot_volum = tot_volum + volum(i)
      END DO
...
C read time and temperature history from data file
...
C calculate tmax, tmin, and dt from read in data
      ...
      ! spaced time history
      call interp_ball(temp_init,time_init,counter,
     *       temp,time,nt)
...
C exact ft
      ...
      DO i=1,nnode
         ft(i)=f_BALL(xnode(i),ynode(i),znode(i))
      END DO
...
C     local variables
      ...
      END SUBROUTINE GKD_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJP_BALL(L0)
      USE direct_procedures_ball
      IMPLICIT NONE
...
40       END DO
      ...
      END SUBROUTINE GJP_BALL
      
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJE_BALL(L0,LL)
      USE direct_procedures_ball
      IMPLICIT NONE
      INTEGER,INTENT(IN) :: L0
...
30    END DO
      ...
      END SUBROUTINE GJE_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QEB_BALL(A,un,uo)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C initialization
      ...
      DO LE=1,nelem
         CALL UEF_BALL(LE,E,F)
         DO NI=1,4
            CALL GF_BALL(NI,LE,FE,uo)
            I=gnode(NI,LE)
            un(I)=un(I)+FE
            DO NJ=1,NI
               J=gnode(NJ,LE)
               CALL GK_BALL(NI,NJ,LE,Aij,E,F)
               CALL AK_BALL(I,J,Aij,A)
            END DO
         END DO
      END DO
      
      END SUBROUTINE QEB_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 

      SUBROUTINE UEF_BALL(LE,E,F)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE UEF_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GF_BALL(NI,LE,FE,uo)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GF_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GK_BALL(NI,NJ,LE,Aij,E,F)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GK_BALL
 
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE AK_BALL(i,j,Aij,A)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE AK_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CRS_BALL(A,W)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CRS_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BDE_BALL(A,un)
      USE direct_procedures_ball
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(ns),INTENT(INOUT) :: A
...
C local variables
	 ...
	 i=gboun(LE)
      ...
      END SUBROUTINE BDE_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QMR_BALL(W,un)
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      REAL(KIND=r8) :: Vnorm_BALL,Cnorm_BALL
...
10    DO i=1,nnode
      ...
      END DO
      call Mult_BALL(W,P,Av)
      eps1=Cnorm_BALL(P,Av)
      eps2=eps1/del
      DO i=1,nnode
         U(i)=Av(i)-eps2*U(i)
      END DO

      rho2=SQRT(Vnorm_BALL(U))
...
c         WRITE(6,*) 'Convergence Achieved! '
      ...
      END SUBROUTINE QMR_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE Mult_BALL(W,un,Av)
      USE direct_procedures_ball
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(nz),INTENT(IN) :: W
...
C local variables
      ...
      END SUBROUTINE Mult_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CLEANUP_BALL
      USE direct_procedures_ball
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CLEANUP_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Vnorm_BALL(R)
      USE direct_procedures_ball
      REAL(KIND=r8),DIMENSION(nnode),INTENT(IN) :: R
      REAL(KIND=r8) :: Vnorm_BALL
...
C local variables
      ...
      END FUNCTION Vnorm_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Cnorm_BALL(X,Y)
      USE direct_procedures_ball
      REAL(KIND=r8) :: Cnorm_BALL
...
! 
      ...
      IMPLICIT NONE
      REAL(KIND=r8) :: t
      REAL(KIND=r8) :: q_BALL,Te_BALL
	  
      
      q_BALL=D0*EXP(-Ea/(Gc*(t+273.15)))

      q_BALL=(q_BALL*(L/rad**2.))                ! Scale diffusivity

!       q=D0*EXP(-Ea/(Gc*Te_BALL(t)))
...
! 
!       REAL(KIND=r8) :: Te_BALL
! 
!       Te_BALL=1.0
! 
!       END FUNCTION Te_BALL

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION f_BALL(x,y,z)
!
! radiogenic source
!
      USE direct_procedures_ball
      ...
      r0=r0/rad                                 ! Find distance on unit sphere with same ratio as actual sphere
      r=SQRT(x*x+y*y+z*z)                       ! Current distance of point from center of sphere
      ...
      if (alpha_ejec .eq. 1) then
        IF (r .LE. (1.0_r8-r0)) THEN              ! If the current distance is closer to the center than 20 microns from edge
           ...
           f_BALL=f_BALL*L
        ELSE
           f_BALL=0.5_r8*Prate*(1.0_r8-r)/r0+     ! Linearly decreae prod rate approaching edge of model to half
     *            0.5_r8*Prate
           f_BALL=f_BALL*L
        END IF
      else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE interp_BALL(tp0,ti0,num0,tpf,tif,numf)
