...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE MAINDRIVER_HEXCYLINDER (tot_conc,D00,Ea0,nt0,
     *           therm_hist0,ages,conc_hist,nnode0,rad0,height,
     *           time_out,alpha_ejec0,diffuse,min_diff)
!
! main program 
! 
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
!
...
C local variables
      ...
      write(6,*) 'Processing 3-D Hexagonal Cylinder H:W=1:1'
...
C parameters initilization
      ...
      CALL SETUP_HEXCYLINDER
      CALL GKD_HEXCYLINDER
...
C forward solver
      ...
      DO it=1,nt
         ...
         if (diffuse2 .eq. 1) then
           CALL QEB_HEXCYLINDER(A,ut(it,:),ut(it-1,:))
           CALL BDE_HEXCYLINDER(A,ut(it,:))
           CALL CRS_HEXCYLINDER(A,W)
           CALL QMR_HEXCYLINDER(W,ut(it,:))
           jt=it
         else
           do j=1,nnode
...
!       tot_volum=tot_volum*side**2.*zamp
     ...
     *                  (vol2/tot_volum)*var_rate
      ...
      DO i=1,nt
        ...
        OPEN(unit=80,file='hexcylinder/He_tec'//i_char//'.dat',
     *       status='unknown')

        write (80,'(A)',ADVANCE="no") 'TITLE = "bambHe Output -'
        write (80,*) ' Hexcylinder - Time: ',time(i),'s"'
...
C clean up variables
      ...
      CALL CLEANUP_HEXCYLINDER

      RETURN
      END SUBROUTINE MAINDRIVER_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE SETUP_HEXCYLINDER
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      REAL(KIND=r8) :: q_HEXCYLINDER,f_HEXCYLINDER,tp
...
C read data
      ...
      OPEN(unit=81,file='hexcylinder/nnes_hexcylinder.dat')
...
C read data from Matlab
      ...
      OPEN(unit=82,file='hexcylinder/xyzn_hexcylinder.dat')
      DO i=1,nnode
	 READ(82,*) xnode(i),ynode(i),znode(i)
      END DO
      CLOSE(unit=82)

      OPEN(unit=83,file='hexcylinder/node_hexcylinder.dat')
      DO i=1,nelem
         READ(83,*) gnode(1,i),gnode(2,i),gnode(3,i),gnode(4,i)
      END DO
      CLOSE(unit=83)

      OPEN(unit=84,file='hexcylinder/sfnd_hexcylinder.dat')
      DO i=1,nsfem
	 READ(84,*) gsfnd(1,i),gsfnd(2,i),gsfnd(3,i)
      END DO
      CLOSE(unit=84)

      OPEN(unit=85,file='hexcylinder/volu_hexcylinder.dat')
      DO i=1,nelem
	 READ(85,*) volum(i)
         tot_volum = tot_volum + volum(i)
      END DO
...
C calculate tmax, tmin, and dt from read in data
      ...
      ! spaced time history
      call interp_hexcylinder(temp_init,time_init,counter,
     *       temp,time,nt)

C exact qt
      DO i=0,nt
	 qt(i)=q_HEXCYLINDER(temp(i))
      END DO
...
C exact ft
      ...
      DO i=1,nnode
         ft(i)=f_HEXCYLINDER(xnode(i),ynode(i),znode(i))
      END DO

C compute the boundary nodal point label

      CALL BOUN_HEXCYLINDER

      END SUBROUTINE SETUP_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BOUN_HEXCYLINDER
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variable
	 ...
	 END IF
      ...
      END SUBROUTINE BOUN_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GKD_HEXCYLINDER
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C     local variables
      ...
      diag(1)=0
      DO L0=1,nnode
         CALL GJP_HEXCYLINDER(L0)
         LI=L0-JP(1)
         diag(L0+1)=diag(L0)+LI+1
      END DO
      ns=diag(nnode+1)

      END SUBROUTINE GKD_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJP_HEXCYLINDER(L0)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
40       END DO
      ...
      END SUBROUTINE GJP_HEXCYLINDER
      
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJE_HEXCYLINDER(L0,LL)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
      INTEGER,INTENT(IN) :: L0
...
30    END DO
      ...
      END SUBROUTINE GJE_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QEB_HEXCYLINDER(A,un,uo)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C initialization
      ...
      DO LE=1,nelem
         CALL UEF_HEXCYLINDER(LE,E,F)
         DO NI=1,4
            CALL GF_HEXCYLINDER(NI,LE,FE,uo)
            I=gnode(NI,LE)
            un(I)=un(I)+FE
            DO NJ=1,NI
               J=gnode(NJ,LE)
               CALL GK_HEXCYLINDER(NI,NJ,LE,Aij,E,F)
               CALL AK_HEXCYLINDER(I,J,Aij,A)
            END DO
         END DO
      END DO
      
      END SUBROUTINE QEB_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 

      SUBROUTINE UEF_HEXCYLINDER(LE,E,F)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE UEF_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GF_HEXCYLINDER(NI,LE,FE,uo)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GF_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GK_HEXCYLINDER(NI,NJ,LE,Aij,E,F)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GK_HEXCYLINDER
 
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE AK_HEXCYLINDER(i,j,Aij,A)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE AK_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CRS_HEXCYLINDER(A,W)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CRS_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BDE_HEXCYLINDER(A,un)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(ns),INTENT(INOUT) :: A
...
C local variables
	 ...
	 i=gboun(LE)
      ...
      END SUBROUTINE BDE_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QMR_HEXCYLINDER(W,un)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
10    DO i=1,nnode
      ...
      del=Cnorm_HEXCYLINDER(U,T)

      DO i=1,nnode
         P(i)=T(i)-(rho1*del/eps0)*P(i)
      END DO
      call Mult_HEXCYLINDER(W,P,Av)
      eps1=Cnorm_HEXCYLINDER(P,Av)
      eps2=eps1/del
      DO i=1,nnode
         U(i)=Av(i)-eps2*U(i)
      END DO

      rho2=SQRT(Vnorm_HEXCYLINDER(U))
      ...
      iter=iter+1
      Rss=SQRT(Vnorm_HEXCYLINDER(R)/Vm)
...
c         WRITE(6,*) 'Convergence Achieved! '
      ...
      END SUBROUTINE QMR_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE Mult_HEXCYLINDER(W,un,Av)
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(nz),INTENT(IN) :: W
...
C local variables
      ...
      END SUBROUTINE Mult_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CLEANUP_HEXCYLINDER
      USE direct_procedures_hexcylinder
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CLEANUP_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Vnorm_HEXCYLINDER(R)
      USE direct_procedures_hexcylinder
      REAL(KIND=r8),DIMENSION(nnode),INTENT(IN) :: R
      REAL(KIND=r8) :: Vnorm_HEXCYLINDER
...
C local variables
      ...
      DO i=1,nnode
         sum=sum+ABS(R(i))**2
      END DO
      Vnorm_HEXCYLINDER=sum
      
      END FUNCTION Vnorm_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Cnorm_HEXCYLINDER(X,Y)
      USE direct_procedures_hexcylinder
      REAL(KIND=r8) :: Cnorm_HEXCYLINDER
...
C local variables
      ...
      DO i=1,nnode
         sum=sum+X(i)*Y(i)
      END DO
      Cnorm_HEXCYLINDER=sum
      
      END FUNCTION Cnorm_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION q_HEXCYLINDER(t)
!
...
! 
      ...
      REAL(KIND=r8) :: q_HEXCYLINDER,Te_HEXCYLINDER

	  
      q_HEXCYLINDER=D0*EXP(-Ea/(Gc*(t+273.15)))
      q_HEXCYLINDER=q_HEXCYLINDER*(L/amp**2.)
!       q=D0*EXP(-Ea/(Gc*Te_HEXCYLINDER(t)))

      END FUNCTION q_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!       FUNCTION Te_HEXCYLINDER(t)
! !
! ! temperature history
! !
!       USE direct_procedures_hexcylinder
...
! 
!       REAL(KIND=r8) :: Te_HEXCYLINDER
! 
!       Te_HEXCYLINDER=1.0
! 
!       END FUNCTION Te_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION f_HEXCYLINDER(x,y,z)
!
! radiogenic source
!
      USE direct_procedures_hexcylinder
      ...
      REAL(KIND=r8) :: r,r0,f_HEXCYLINDER,angle,unit_dist
      ...
      if (alpha_ejec .eq. 1) then
      IF (r .LE. (real_dist-r0) .and. abs(z*amp).le.(zamp-20.e-6)) THEN         ! If the current distance is closer to the center than 20 microns from edge
          f_HEXCYLINDER=Prate
          f_HEXCYLINDER=f_HEXCYLINDER*L                                           ! Scale production rate
        ELSE
          fstore1=0.5_r8*Prate*(zamp-abs(z*amp))/(20.e-6)+                        ! Set production rate to be linear function based on vertical distance from edge
     *            0.5_r8*Prate
     ...
     *            0.5_r8*Prate
          ...
          if (fstore1 .lt. fstore2) then
            f_HEXCYLINDER=fstore1
          else
            f_HEXCYLINDER=fstore2
          endif
        ...
        f_HEXCYLINDER=Prate
        f_HEXCYLINDER=f_HEXCYLINDER*L
      endif

      END FUNCTION f_HEXCYLINDER

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      ! Subroutine that linearly interpolates a temperature history
      ! onto a linearly spaced time history
      SUBROUTINE interp_HEXCYLINDER(tp0,ti0,num0,tpf,tif,numf)
     ...
     *               (tif(i)-ti0(j))
      ...
      END SUBROUTINE interp_HEXCYLINDER
