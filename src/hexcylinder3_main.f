...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE MAINDRIVER_hexcylinder3 (tot_conc,D00,Ea0,nt0,
     *           therm_hist0,ages,conc_hist,nnode0,rad0,height,
     *           time_out,alpha_ejec0,diffuse,min_diff)
!
! main program 
! 
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
!
...
C local variables
      ...
      write(6,*) 'Processing 3-D Hexagonal Cylinder H:W=3:1'
...
C parameters initilization
      ...
      CALL SETUP_hexcylinder3
      CALL GKD_hexcylinder3
...
C forward solver
      ...
      DO it=1,nt
         ...
         if (diffuse2 .eq. 1) then
           CALL QEB_hexcylinder3(A,ut(it,:),ut(it-1,:))
           CALL BDE_hexcylinder3(A,ut(it,:))
           CALL CRS_hexcylinder3(A,W)
           CALL QMR_hexcylinder3(W,ut(it,:))
           jt=it
         else
           do j=1,nnode
...
!       tot_volum=tot_volum*side**2.*zamp
     ...
     *                  (vol2/tot_volum)*var_rate
      ...
      DO i=1,nt
        ...
        OPEN(unit=80,file='hexcylinder_3to1/He_tec'//i_char//'.dat',
     *       status='unknown')

        write (80,'(A)',ADVANCE="no") 'TITLE = "bambHe Output -'
        write (80,*) ' Hexcylinder 3:1 - Time: ',time(i),' s"'
...
C clean up variables
      ...
      CALL CLEANUP_hexcylinder3

      RETURN
      END SUBROUTINE MAINDRIVER_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE SETUP_hexcylinder3
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      REAL(KIND=r8) :: q_hexcylinder3,f_hexcylinder3,tp
...
C read data
      ...
      OPEN(unit=81,file='hexcylinder_3to1/nnes_hexcylinder.dat')
...
C read data from Matlab
      ...
      OPEN(unit=82,file='hexcylinder_3to1/xyzn_hexcylinder.dat')
      DO i=1,nnode
	 READ(82,*) xnode(i),ynode(i),znode(i)
      END DO
      CLOSE(unit=82)

      OPEN(unit=83,file='hexcylinder_3to1/node_hexcylinder.dat')
      DO i=1,nelem
         READ(83,*) gnode(1,i),gnode(2,i),gnode(3,i),gnode(4,i)
      END DO
      CLOSE(unit=83)

      OPEN(unit=84,file='hexcylinder_3to1/sfnd_hexcylinder.dat')
      DO i=1,nsfem
	 READ(84,*) gsfnd(1,i),gsfnd(2,i),gsfnd(3,i)
      END DO
      CLOSE(unit=84)

      OPEN(unit=85,file='hexcylinder_3to1/volu_hexcylinder.dat')
      DO i=1,nelem
	 READ(85,*) volum(i)
         tot_volum = tot_volum + volum(i)
      END DO
...
C calculate tmax, tmin, and dt from read in data
      ...
      ! spaced time history
      call interp_hexcylinder3(temp_init,time_init,counter,
     *       temp,time,nt)

C exact qt
      DO i=0,nt
	 qt(i)=q_hexcylinder3(temp(i))
      END DO
...
C exact ft
      ...
      DO i=1,nnode
         ft(i)=f_hexcylinder3(xnode(i),ynode(i),znode(i))
      END DO

C compute the boundary nodal point label

      CALL BOUN_hexcylinder3

      END SUBROUTINE SETUP_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BOUN_hexcylinder3
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variable
	 ...
	 END IF
      ...
      END SUBROUTINE BOUN_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GKD_hexcylinder3
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C     local variables
      ...
      diag(1)=0
      DO L0=1,nnode
         CALL GJP_hexcylinder3(L0)
         LI=L0-JP(1)
         diag(L0+1)=diag(L0)+LI+1
      END DO
      ns=diag(nnode+1)

      END SUBROUTINE GKD_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJP_hexcylinder3(L0)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
40       END DO
      ...
      END SUBROUTINE GJP_hexcylinder3
      
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJE_hexcylinder3(L0,LL)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
      INTEGER,INTENT(IN) :: L0
...
30    END DO
      ...
      END SUBROUTINE GJE_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QEB_hexcylinder3(A,un,uo)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C initialization
      ...
      DO LE=1,nelem
         CALL UEF_hexcylinder3(LE,E,F)
         DO NI=1,4
            CALL GF_hexcylinder3(NI,LE,FE,uo)
            I=gnode(NI,LE)
            un(I)=un(I)+FE
            DO NJ=1,NI
               J=gnode(NJ,LE)
               CALL GK_hexcylinder3(NI,NJ,LE,Aij,E,F)
               CALL AK_hexcylinder3(I,J,Aij,A)
            END DO
         END DO
      END DO
      
      END SUBROUTINE QEB_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 

      SUBROUTINE UEF_hexcylinder3(LE,E,F)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE UEF_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GF_hexcylinder3(NI,LE,FE,uo)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GF_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GK_hexcylinder3(NI,NJ,LE,Aij,E,F)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GK_hexcylinder3
 
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE AK_hexcylinder3(i,j,Aij,A)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE AK_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CRS_hexcylinder3(A,W)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CRS_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BDE_hexcylinder3(A,un)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(ns),INTENT(INOUT) :: A
...
C local variables
	 ...
	 i=gboun(LE)
      ...
      END SUBROUTINE BDE_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QMR_hexcylinder3(W,un)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      REAL(KIND=r8) :: Vnorm_hexcylinder3,Cnorm_hexcylinder3
      REAL(KIND=r8) :: eps0,eps1,eta,del,eps2
      REAL(KIND=r8),DIMENSION(nnode) :: Av,P,U,R,T     

      iter=0       
      Vm=Vnorm_hexcylinder3(un)
      ...
      rho1=SQRT(Vnorm_hexcylinder3(U))
      c0=1.0_r8
      eps0=1.0_r8
      hu0=0.0_r8
      eta=-1.0_r8
...
10    DO i=1,nnode
      ...
      del=Cnorm_hexcylinder3(U,T)

      DO i=1,nnode
         P(i)=T(i)-(rho1*del/eps0)*P(i)
      END DO
      call Mult_hexcylinder3(W,P,Av)
      eps1=Cnorm_hexcylinder3(P,Av)
      eps2=eps1/del
      DO i=1,nnode
         U(i)=Av(i)-eps2*U(i)
      END DO

      rho2=SQRT(Vnorm_hexcylinder3(U))
      ...
      iter=iter+1
      Rss=SQRT(Vnorm_hexcylinder3(R)/Vm)
...
c         WRITE(6,*) 'Convergence Achieved! '
      ...
      END SUBROUTINE QMR_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE Mult_hexcylinder3(W,un,Av)
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(nz),INTENT(IN) :: W
...
C local variables
      ...
      END SUBROUTINE Mult_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CLEANUP_hexcylinder3
      USE direct_procedures_hexcylinder3
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CLEANUP_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Vnorm_hexcylinder3(R)
      USE direct_procedures_hexcylinder3
      REAL(KIND=r8),DIMENSION(nnode),INTENT(IN) :: R
      REAL(KIND=r8) :: Vnorm_hexcylinder3
...
C local variables
      ...
      DO i=1,nnode
         sum=sum+ABS(R(i))**2
      END DO
      Vnorm_hexcylinder3=sum
      
      END FUNCTION Vnorm_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Cnorm_hexcylinder3(X,Y)
      USE direct_procedures_hexcylinder3
      REAL(KIND=r8) :: Cnorm_hexcylinder3
...
C local variables
      ...
      DO i=1,nnode
         sum=sum+X(i)*Y(i)
      END DO
      Cnorm_hexcylinder3=sum
      
      END FUNCTION Cnorm_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION q_hexcylinder3(t)
!
...
! 
      ...
      REAL(KIND=r8) :: q_hexcylinder3,Te_hexcylinder3

	  
      q_hexcylinder3=D0*EXP(-Ea/(Gc*(t+273.15)))
      q_hexcylinder3=q_hexcylinder3*(L/amp**2.)
!       q=D0*EXP(-Ea/(Gc*Te_hexcylinder3(t)))

      END FUNCTION q_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!       FUNCTION Te_hexcylinder3(t)
! !
! ! temperature history
! !
!       USE direct_procedures_hexcylinder3
...
! 
!       REAL(KIND=r8) :: Te_hexcylinder3
! 
!       Te_hexcylinder3=1.0
! 
!       END FUNCTION Te_hexcylinder3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION f_HEXCYLINDER3(x,y,z)
!
! radiogenic source
!
      USE direct_procedures_hexcylinder3
      ...
      REAL(KIND=r8) :: r,r0,f_HEXCYLINDER3,angle,unit_dist
      ...
      if (alpha_ejec .eq. 1) then
      IF (r.LE.(real_dist-r0) .and. abs(3.*z*amp).le.(zamp-20.e-6)) THEN         ! If the current distance is closer to the center than 20 microns from edge
          f_HEXCYLINDER3=Prate
          f_HEXCYLINDER3=f_HEXCYLINDER3*L                                          ! Scale production rate
        ELSE
          fstore1=0.5_r8*Prate*(zamp-abs(3.*z*amp))/(20.e-6)+                      ! Set production rate to be linear function based on vertical distance from edge
     *            0.5_r8*Prate
     ...
     *            0.5_r8*Prate
          ...
          if (fstore1 .lt. fstore2) then
            f_HEXCYLINDER3=fstore1
          else
            f_HEXCYLINDER3=fstore2
          endif
        ...
        f_HEXCYLINDER3=Prate
        f_HEXCYLINDER3=f_HEXCYLINDER3*L
      endif

      END FUNCTION f_HEXCYLINDER3

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      ! Subroutine that linearly interpolates a temperature history
      ! onto a linearly spaced time history
      SUBROUTINE interp_hexcylinder3(tp0,ti0,num0,tpf,tif,numf)
     ...
     *               (tif(i)-ti0(j))
      ...
      END SUBROUTINE interp_hexcylinder3
