...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE MAINDRIVER_HEXDISC (tot_conc,D00,Ea0,nt0,
     *           therm_hist0,ages,conc_hist,nnode0,rad0,
     *           time_out,alpha_ejec0,diffuse,min_diff)
!
! main program 
! 
      USE direct_procedures_hexdisc
      IMPLICIT NONE
!
...
C local variables
      ...
      REAL(KIND=r8) :: const_rate,var_rate,Umc,Thmc
      REAL(KIND=r8) :: area1,area2,side2,rad0,min_diff
      INTEGER :: err,i,j,k,nt0,concFlag,nnode0,ft_count
      INTEGER :: alpha_ejec0,diffuse,jt,diffuse2
      CHARACTER i_char*4,therm_hist0*100

      write(6,*) 'Processing 2-D Hexagonal Disc'
...
C forward solver
      ...
      DO it=1,nt
         ...
         if (diffuse2 .eq. 1) then
           CALL QEB_HEXDISC(A,ut(it,:),ut(it-1,:))
           CALL BDE_HEXDISC(A,ut(it,:))
           CALL CRS_HEXDISC(A,W)
           CALL QMR_HEXDISC(W,ut(it,:))
           jt=it
         else
           do j=1,nnode
...
C added 05/08 by cspath
      ...
      ! Side length of actual hexagon
      side=sqrt((amp*cos(0.)-amp*cos((2.*pi)/6.))**2.+
     *     (amp*sin(0.)-amp*sin((2.*pi)/6.))**2.)

      ! Scale hexagon back to original dimensions
      xnode=xnode*amp
      ynode=ynode*amp
      area=area*side**2.
      time=time*L
...
!       tot_area=tot_area*side**2.
      tot_area=0.
      do i=1,nelem
        tot_area=tot_area+area(i)
      enddo

      ! Side length of hexagon face that is 20 microns in on all sides
     ...
     *        (amp*sin((2*pi)/6.)-20.e-6_r8*sin((2*pi)/6.)))**2.)
      ...
      area1 = 3.*sqrt(3.)/2.*side2**2.          ! Area of inner disc that contains points greater than 20 microns from edge
...
C clean up variables
      ...
      CALL CLEANUP_HEXDISC

      RETURN
      END SUBROUTINE MAINDRIVER_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE SETUP_HEXDISC
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      REAL(KIND=r8) :: q_HEXDISC,f_HEXDISC,tp
...
C read data
      ...
      OPEN(unit=81,file='hexdisc/nnee_hexdisc.dat')
...
C read data from Matlab
      ...
      OPEN(unit=82,file='hexdisc/xy_hexdisc.dat')
      DO i=1,nnode
	 READ(82,*) xnode(i),ynode(i)
      END DO
      CLOSE(unit=82)

      OPEN(unit=83,file='hexdisc/node_hexdisc.dat')
      DO i=1,nelem
         READ(83,*) gnode(1,i),gnode(2,i),gnode(3,i)
      END DO
      CLOSE(unit=83)

      OPEN(unit=84,file='hexdisc/edge_hexdisc.dat')
      DO i=1,nedge
	 READ(84,*) gedge(1,i),gedge(2,i)
      END DO
      CLOSE(unit=84)

      OPEN(unit=85,file='hexdisc/area_hexdisc.dat')
      DO i=1,nelem
	 READ(85,*) area(i)
         tot_area = tot_area + area(i)
      END DO
...
C calculate tmax, tmin, and dt from read in data
      ...
      ! spaced time history
      call interp_HEXDISC(temp_init,time_init,counter,
     *       temp,time,nt)

C exact qt

      DO i=0,nt
        qt(i)=q_HEXDISC(temp(i))
      END DO
...
C exact ft
      ...
      DO i=1,nnode
         ft(i)=f_HEXDISC(xnode(i),ynode(i))
      END DO
...
C compute the boundary nodal point label
      ...
      END SUBROUTINE SETUP_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BOUN_HEXDISC
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variable
	 ...
	 END IF
      ...
      END SUBROUTINE BOUN_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GKD_HEXDISC
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C     local variables
      ...
      END SUBROUTINE GKD_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJP_HEXDISC(L0)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
40       END DO
      ...
      END SUBROUTINE GJP_HEXDISC
      
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GJE_HEXDISC(L0,LL)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
      INTEGER,INTENT(IN) :: L0
...
30    END DO
      ...
      END SUBROUTINE GJE_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QEB_HEXDISC(A,un,uo)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C initialization
      ...
      DO LE=1,nelem
         CALL UEF_HEXDISC(LE,E,F)
         DO NI=1,3
            CALL GF_HEXDISC(NI,LE,FE,uo)
            I=gnode(NI,LE)
            un(I)=un(I)+FE
            DO NJ=1,NI
               J=gnode(NJ,LE)
               CALL GK_HEXDISC(NI,NJ,LE,Aij,E,F)
               CALL AK_HEXDISC(I,J,Aij,A)
            END DO
         END DO
      END DO
      
      END SUBROUTINE QEB_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 

      SUBROUTINE UEF_HEXDISC(LE,E,F)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE UEF_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GF_HEXDISC(NI,LE,FE,uo)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GF_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE GK_HEXDISC(NI,NJ,LE,Aij,E,F)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE GK_HEXDISC
 
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE AK_HEXDISC(i,j,Aij,A)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE AK_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CRS_HEXDISC(A,W)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CRS_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE BDE_HEXDISC(A,un)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(ns),INTENT(INOUT) :: A
...
C local variables
	 ...
	 i=gboun(LE)
      ...
      END SUBROUTINE BDE_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE QMR_HEXDISC(W,un)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      REAL(KIND=r8) :: Vnorm_HEXDISC,Cnorm_HEXDISC
...
10    DO i=1,nnode
      ...
      del=Cnorm_HEXDISC(U,T)
      DO i=1,nnode
         P(i)=T(i)-(rho1*del/eps0)*P(i)
      END DO
      call Mult_HEXDISC(W,P,Av)
      eps1=Cnorm_HEXDISC(P,Av)
      eps2=eps1/del
      DO i=1,nnode
         U(i)=Av(i)-eps2*U(i)
      END DO

      rho2=SQRT(Vnorm_HEXDISC(U))
...
c         WRITE(6,*) 'Convergence Achieved! '
      ...
      END SUBROUTINE QMR_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE Mult_HEXDISC(W,un,Av)
      USE direct_procedures_hexdisc
      IMPLICIT NONE
      REAL(KIND=r8),DIMENSION(nz),INTENT(IN) :: W
...
C local variables
      ...
      END SUBROUTINE Mult_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE CLEANUP_HEXDISC
      USE direct_procedures_hexdisc
      IMPLICIT NONE
...
C local variables
      ...
      END SUBROUTINE CLEANUP_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Vnorm_HEXDISC(R)
      USE direct_procedures_hexdisc
      REAL(KIND=r8),DIMENSION(nnode),INTENT(IN) :: R
      REAL(KIND=r8) :: Vnorm_HEXDISC
...
C local variables
      ...
      END FUNCTION Vnorm_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION Cnorm_HEXDISC(X,Y)
      USE direct_procedures_hexdisc
      REAL(KIND=r8) :: Cnorm_HEXDISC
...
C local variables
      ...
      END FUNCTION Cnorm_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION q_HEXDISC(t)
!
...
C local variables
      REAL(KIND=r8) :: q_HEXDISC,Te_HEXDISC
!       REAL(KIND=r8) :: t0,q
...
!       END IF
      ...
      q_HEXDISC=D0*EXP(-Ea/(Gc*(t+273.15)))
      q_HEXDISC=q_HEXDISC*(L/amp**2.)

      END FUNCTION q_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      FUNCTION f_HEXDISC(x,y)
!
! radiogenic source
!
      USE direct_procedures_hexdisc
...
C local variable
      ...
      REAL(KIND=r8) :: r,r0,f_HEXDISC,unit_dist,angle,dr
      ...
      unit_dist=(sqrt(3.)/2.)/cos(angle)                ! Distance on unit hexagon to the edge in x,y plane
      real_dist=amp*unit_dist                           ! Distance on actual hexagon
      ...
      if (alpha_ejec .eq. 1) then
        IF (r .LE. (real_dist-r0)) THEN                   ! If the current distance is closer to the center than 20 microns from edge
           f_HEXDISC=Prate
           f_HEXDISC=f_HEXDISC*L                          ! Scale production rate
        ELSE
           f_HEXDISC=0.5_r8*Prate*(real_dist-r)/r0+	! Linearly decrease production rate from 20 microns in to be half at edge
     *                 0.5_r8*Prate
           f_HEXDISC=f_HEXDISC*L                          ! Scale production rate
        END IF
      else
        f_HEXDISC=Prate
        f_HEXDISC=f_HEXDISC*L
      endif

      END FUNCTION f_HEXDISC

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      ! Subroutine that linearly interpolates a temperature history
      ! onto a linearly spaced time history
      SUBROUTINE interp_HEXDISC(tp0,ti0,num0,tpf,tif,numf)
